const FlavorsBYSKU = {
  // 5-Day
  PR067A: "Expanded Variety 4",
  "PR067A-SUB": "Expanded Variety 4",
  "3PR067A": "Expanded Variety 4",
  PR051A: "Soup flavor Variety 3",
  "PR051A-SUB": "Soup flavor Variety 3",
  "3PR051A": "Soup flavor Variety 3",
  PR005A: "Original soup flavor",
  "PR005A-SUB": "Original soup flavor",
  "3PR005A": "Original soup flavor",
  PR006A: "Soup flavor Variety 2",
  "PR006A-SUB": "Soup flavor Variety 2",
  "3PR006A": "Soup flavor Variety 2",
  PR068A: "Next Gen",
  "PR068A-SUB": "Next Gen",
  // Nutrition Bar
  FB111617EU: "Variety Pack",
  FB1112EU: "Nuts & Honey",
  FB1612EU: "Coconut Macadamia",
  FB1712EU: "Chocolate Chips",
  // ReSet
  PR037: "Original soup flavor",
  PR038: "Original soup flavor",
  PR039: "Original soup flavor",
  PR050: "Soup flavor Variety 2",
  "PR050-X2": "Soup flavor Variety 2",
  "PR050-X3": "Soup flavor Variety 2",
  PR062: "Soup flavor Variety 3",
  PR063: "Soup flavor Variety 4",
  PR065: "Soup flavor Variety 5",
  PR071:"Next Gen Variety 1",
  "PR071-SUB": "Next Gen Variety 2",
  PR072:"Next Gen Variety 2",
  "PR072-SUB": "Next Gen Variety 2",
  // Other products
...
const FlavorsDescriptionBYSKU = {
  // 5-Day
  PR067A:
    "Minestrone Soup, Green Pea & Chives Soup, Chickpea & Leeks Soup, Lentil Curry Soup, Red Pepper & Oninion Soup, Tomato Soup, Carrot Ginger Soup",
  "3PR067A":
    "Minestrone Soup, Green Pea & Chives Soup, Chickpea & Leeks Soup, Lentil Curry Soup, Red Pepper & Oninion Soup, Tomato Soup, Carrot Ginger Soup",
  PR051A:
    "Ginger and carrot, lentils and curry, peas and chives, peppers and onion, chickpeas and leeks",
  "3PR051A":
    "Ginger and carrot, lentils and curry, peas and chives, peppers and onion, chickpeas and leeks",
  PR005A: "Minestrone, Tomato, Mushroom, Vegetable, Minestrone Quinoa",
  "3PR005A": "Minestrone, Tomato, Mushroom, Vegetable, Minestrone Quinoa",
  PR006A:
    "Black Bean, Tomato, Spinach & White Bean, Butternut Squash Quinoa, Butternut Squash",
  "3PR006A":
    "Black Bean, Tomato, Spinach & White Bean, Butternut Squash Quinoa, Butternut Squash",
  PR068A:
    "Broccoli & Quinoa, Artichoke & Yellow Tomato, Minestrone, Tomato, Vegetable",
  "PR068A-SUB":
    "Broccoli & Quinoa, Artichoke & Yellow Tomato, Minestrone, Tomato, Vegetable",
  // ReSet
  PR037: "Tomato, Butternut Squash",
  PR038: "Tomato, Butternut Squash",
  PR039: "Tomato, Butternut Squash",
  PR050: "Vegetables and Black Beans",
  "PR050-X2": "Vegetables and Black Beans",
  "PR050-X3": "Vegetables and Black Beans",
  PR062: "Red Pepper, Vegetable",
  PR063: "Carrot Ginger, Lentil",
  PR065: "Peas Chives/ Chickpea",
  // Nutrition Bars
  FB111617EU: "4 x Nuts & Honey, 4 x Coconut Macadamia, 4 x Chocolate Chips",
...
const SingleOption = {
  template: /*html*/ `
      ...
      <span
  v-if="!isMain && variant && variant.option1 === 'Variety 2' && variant.option2 && variant.option2.toLowerCase().includes('one time')"
  class="!hidden z-20 p-1 px-[6px] lg:block bg-[#73984A] text-white rounded-tr rounded-bl !text-[14px] absolute !top-[0px] !right-[0px] font-bold">
  Ⰶ BUY 1, GET 1 FREE
</span>

<span
  v-else-if="!isMain && variant && variant.option2 && savingValue && variant.option2.toLowerCase().includes('subscri')"
  ...
  + {{savingPercentage}}% OFF
        ...
        <img v-if="showVariantImage && !isMain" class="w-12 hidden max-h-12 md:max-h-20 rounded" :src="showVariantImage" alt="Featured Image" />
        <div class="max-w-full flex-1">
          <span class="flex justify-center">
            <span v-if="isMain" class="  text-[12px] md:text-[14px]  uppercase font-semibold text-left block"
             ...
             <div class="flex items-center gap-x-1" >
              ...
              <div v-if="!isMain">
                ...
                <div :class='["flex flex-wrap items-center justify-between h-[28px]"]'>
                  <div class="flex top-[-14px] relative">
                    <span v-if="isSubscription" class="!block text-left font-semibold  !text-[#73984A] text-[12px]">
                      Delivers every
                    </span>
                    <span v-else class="!block text-left font-semibold  !text-[#73984A] text-[12px]">
                      One time purchase
                    </span>
  ...
  template: /*html*/ `
       <div
          ...
          >
            ...
            <button
              :class="[
                checked && 'option-0 normal !bg-transparent has-saving text-nowrap',
                'w-full'
              ]"
              @click="chooseType(option_name); toggle_selected"
                     >
      ...
      <div 
        :class="[
          'buttonsfloat lg:max-w-[600px] relative em7gzp67 z-20 border-prolon_green',
        checked ? 'block' : 'hidden', isBundle ? 'move-right' : 'true',
        ]"
        role="listbox"
      >
            <button
...
const SingleProductSectionShopNowUpsell = {
  template: /*html*/ `
    ...
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto"><div class="flex min-h-full items-end justify-center p-0  text-center sm:items-center sm:p-0"><!--
        ...
        --><div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"  @click="closeQuickView"></div><!--  close svg -->
          ...
          <div class="fixed bottom-0 md:relative transform  text-left  transition-all sm:my-8 sm:w-full sm:max-w-[700px] 2xl:max-w-[60%] w-full m:h-[90vh]  sm:align-middle sm:rounded-lg sm:shadow-xl px-2" 
        ...
        ><span class="sr-only">Close</span><!-- Heroicon name: outline/x --><svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div><div class="pl-wrapper pl-wrapper-product-section h-full bg-white px-4 py-2 overflow-x-hidden overflow-y-scroll loaded_animation"><div class="relative"><div class="mx-auto pt-0  lg:max-w-[1600px] sm:px-[20px] md:px-[0px]"><h1 class="font-avant my-[10px] md:my-[40px] text-[16px] md:text-[28px] text-center text-black"><span class="bg-[#98ca3c] font-semibold inline-block leading-8 md:leading-10 mt-2 md:px-2 md:py-1 text-left text-white">Recommendation:</span> Add Prolon Longevity Protein + <br class="m:hidden">
  Gut Support to extend your results after your fast.</h1>   
       
  <div class="md:grid md:grid-cols-6 lg:grid-cols-6 lg:grid-rows-1 lg:gap-x-8 lg:gap-[30px] xl:gap-[50px]"><div class="md:col-span-3 lg:col-span-4 lg:row-end-1 lg:px-0"><div class="flex flex-col"><div class="w-full aspect-w-1 aspect-h-1 flex flex-nowrap md:grid md:grid-cols-2 gap-2 md:justify-center md:items-center overflow-x-scroll overflow-y-hidden md:overflow-hidden"><div class="shrink-0  col-span-2 md:!max-w-full  hero__animation-contents relative " style="transition-delay: 1.2s;"><img src="https://cdn.shopify.com/s/files/1/0508/0625/9870/files/ProlonBundleImagesBiome_Protein.png?v=1715197703" class="!w-auto md:!w-full md:!max-w-full min-h-[350px] m:!max-h-[350px] md:!h-full !object-cover" alt="Product image" loading="eager"></div><img class="col-span-2 !hidden !max-w-full !h-auto !object-contain" src="https://cdn.shopify.com/s/files/1/0126/2921/3243/files/pl-Featured.png?v=1690546584&amp;width=2754&amp;height=129"></div></div></div><div class="w-full max-w-3xl mx-auto mt-4 sm:mt-4 md:col-span-3 lg:col-span-5 lg:row-span-2 lg:row-end-2 lg:mt-0 lg:max-w-none product-details lg:sticky lg:top-[60px]   lg:max-h-[850px]   mh900:top-[100px] px-0 md:px-[15px] "><div class="flex flex-col justify-center items-start m:mb-[120px]"><h1  class="w-full text-left text-[15px]  font-bold  text-prolon sm:text-[17px] mt-[10px]">Ultimate duo designed to renourish your body and provide high potency probiotics to maintain your microbiome after Prolon
  5-day fast.
  </h1><div class="flex items-end justify-start gap-3 ">
  <span class="font-bold text-gray-900 text-md mt-2 blocktext-left font-semibold text-gray-900 text-[16px] md:text-[20px]">$99</span> 
  <s class="mt-2 block  text-left font-semibold text-gray-400 text-[14px] md:text-[16px] ">$129</s>
  <span class="bg-[#98ca3c] block font-semibold md:text-[16px] mt-2 px-4 py-1 text-[14px]  text-left text-white">23% OFF </span>
  </div><div class="mt-4 w-full product-description">
  <div  class="w-full text-left text-[15px]  font-bold  text-prolon sm:text-[17px] mt-[10px]">
  L-Protein:
  </div>
  <ol class="mb-4 list-disc">  
          <li> World's first protein with amino acid profile researched to extend longevity by limiting increases in IGF-1</li>  
  ...
  <ol class="mb-4 list-disc">  
          <li> High potency pre- and probiotics to support your fasting microbiome</li>  
  <li>Dual-cap technology with DHA for ultimate gut health</li> 
  <br>
      ...
      store.addToCartWithProperties({
        ...
        cb: () => {
          ...
          try {
            ...
            this.isModalOpen = false;
            if (window.posthog) {
              posthog.capture("quick_view_added_to_cart", {
    ...
    computed: {
      ...
      compare_at_price() {
        if (
          ...
          const price = window.compare_at_prices[this.currentVariant.id] / 100;
          return price.toLocaleString("en-EN", {
            style: "currency",
            currency: "GBP",
          });
        } else {
          return (0).toLocaleString("en-EN", {
            style: "currency",
            currency: "GBP",
          });
        }
