...
(function($){
  ...
  $(function() {
    ...
    $('body').on('click', '.cart__checkout', function() {
      ...
      console.log("Top click from cart");
      return false;
    });
    ...
    $('body').on('click', '.cart__checkout--page', function(evt) {
      ...
      if ($('#CartPageAgree').is(':checked')) {
        console.log("theme click from cart");
      } else {
        alert(theme.strings.cartTermsConfirmation);
        $(this).removeClass('btn--loading');
        return false;
...
sellingPlan: product.selling_plan_allocation ? product.selling_plan_allocation.selling_plan.name : '',
  ...
  theme.headerNav = (function() {
    ...
    function init() {
      ...
      if (config.stickyEnabled) {
        ...
        stickyHeaderCheckFixed();
        $window.on(
          "resize" + config.namespace,
          $.debounce(150, stickyHeaderCheckFixed)
        ); 
    ...
    function stickyHeaderCheck() {
      ...
      $(".site-fixed-spacer").css("height", siteHeaderHeight); 
   ...
   function stickyHeaderCheckFixed() {
     // Disable sticky header if any mega menu is taller than window
     theme.config.stickyHeader = doesMegaMenuFit();
     let siteHeaderHeight = $(".site-header").height();
  ...
  theme.videoModal = function() {
    ...
    var selectors = {
      ...
      vimeo: 'a[href*="vimeo.com/"]'
    };
  
    if (!$(selectors.youtube).length && !$(selectors.mp4Trigger).length && !$(selectors.vimeo).length) {
      return;
    }
    ...
    $(selectors.vimeo).on('click', triggerVimeoModal);
    ...
    function triggerVimeoModal(evt) {
      ...
      evt.preventDefault();
      // https://player.vimeo.com/video/815706031?h=f9c78bf0a1&badge=0&autopause=0&player_id=0&app_id=58479
      let vimeoUrl= $(evt.currentTarget).attr('href');
    
   var option = {
     url:vimeoUrl,
     width: "600",
   }; 
      var player = new Vimeo.Player(selectors.videoHolder.replace('#',''), option);
      modal.open(evt);
      activeVideo = 'vimeo';               
      player.play(); 
    }  
  ...
  theme.Product = (function() {
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      updatePrice: function(evt) {
        ...
        let compareAtPriceFrontend = $(`option[value="${variant.id}"]`).attr('data-compare_at_price_frontend');

        if(compareAtPriceFrontend && compareAtPriceFrontend!=''){
          variant.compare_at_price=compareAtPriceFrontend
        }  
