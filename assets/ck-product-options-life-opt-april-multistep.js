...
const SingleOption = {
  ...
  methods: {
      ...
      chooseFlavor(variant) {
        this.selectedFlavor = variant.option1;
        this.selectedVariant = variant;
      },
      chooseOrderType(variant) {
        this.selectedVariant = variant;
      },
      addToCartFinal() {
        const items = [];
        ...
        if (this.selectedVariant) {
          items.push({
            ...
            quantity: 1,
            ...(this.selectedVariant.selling_plan_allocation ? {
              selling_plan: this.selected_plan?.id
            } : {})
          });
        }

        if (this.selectedFbtProduct) {
          items.push({
            id: this.selectedFbtProduct.variants[0].id,
            quantity: 1
          });
        }

        if (this.selectedUpsellProduct) {
          items.push({
            id: this.selectedUpsellProduct.variants[0].id,
            quantity: 1
          });
        }
  ...
  mounted() {
      // Load FBT and Upsell products by tag
      fetch('/products.json?limit=250')
        .then(res => res.json())
        .then(data => {
          const allProducts = data.products;
          const tagBase = 'reset'; // In futuro dinamico con handle

          this.fbtProducts = allProducts.filter(p =>
            p.tags.includes(`fbt-${tagBase}`)
          );

          this.upsellProducts = allProducts.filter(p =>
            p.tags.includes(`upsell-${tagBase}`)
          );
        });
  ...
  template: /*html*/ `
       <div
          ...
          >
            ...
            <button
              :class="[
                checked && 'option-0 normal !bg-transparent has-saving',
                'w-full'
              ]"
      ...
      <div 
        :class="[
          'buttonsfloat relative em7gzp67 z-20 border-prolon_primary',
...
if (document.querySelector("#ck-product-form")) {
  ...
  data() {
    return {
      ...
      fbtProducts: [],
      upsellProducts: [],
      flavorOptions: [], // <-- riempito dinamicamente, o metti ['Tomato', 'Minestrone'] per test
      orderOptions: ['One Time Purchase', 'Subscribe & Save'],
      stepLabels: ['Flavor', 'Type', 'Extras'],
      addingtoCart: false,
      selected_plan: null,
  ...
  methods: {
    ...
    chooseFlavor(option) {
      this.selectedFlavor = option;

      const variant = this.product.variants.find(v => v.option1 === option);
      if (variant) {
        this.selectedVariant = variant;
      }
    },
    chooseOrderType(option) {
      if (this.selectedVariant) {
        const match = this.product.variants.find(v =>
          v.option1 === this.selectedFlavor && v.option2 === option
        );
    ...
    addToCartFinal() {
      ...
      if (this.selectedVariant) {
        ...
        if (this.selectedVariant.selling_plan_allocation) {
          baseItem.selling_plan = this.selected_plan?.id;
        }
  ...
  mounted() {
    const tagBase = this.product?.handle?.includes('5-day') ? '5-day' : 'reset';
    ...
    fetch('/products.json?limit=250')
      ...
      .then(data => {
        ...
        // Popola flavorOptions univoche (option1)
        const flavorSet = new Set();
        this.product.variants.forEach(v => {
          if (v.option1) flavorSet.add(v.option1);
        });
        this.flavorOptions = Array.from(flavorSet);
      });
  }
