  ...
  <span class="inline-block w-full px-6 py-3 font-medium text-center rounded bg-prolon_orange text-contrast">
    ...
    <span v-else class="flex items-center justify-center gap-2 text-white max-w-prose inherit text-copy">
      ...
      <div>
        ...
        <span class="font-bold" v-if="price" >{{money(price)}}</span>
      </div>
    </span>
  </span>
</button>     
...
<label class="!text-sm !font-normal text-gray-700 mt-1 mb-0" role="none">Frequency: {{selling_plan && selling_plan.name}}</label>
                    </div> 
<button  v-if="!SubscriptionOptionActivated &&subscriptionVariantId &&subscriptionVariantId!=''"   :disabled="addingtoCart" type="button" class="bg-buttonColor mb-3 subscription w-full max-w-[94%]" @click="activateOptions">
  <span class="inline-block w-full px-6 py-3 font-medium text-center rounded bg-black text-contrast">
    ...
    <span  class="flex items-center justify-center gap-2 text-white max-w-prose inherit text-copy">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 20 20"
        fill="currentColor"
...
const   ProductQuickView = {
  ...
  <button    type="button" class="bg-buttonColor mb-3 quick-view-btn w-full" @click="openModal()">
  <span class="inline-block w-full px-6 py-3 font-medium text-center rounded bg-prolon_orange text-contrast">
    ...
    <span v-else class="flex items-center justify-center gap-2 text-white max-w-prose inherit text-copy">
    <svg aria-hidden="true" focusable="false" role="presentation" class="icon icon-search" viewBox="0 0 64 64"><defs></defs><path class="cls-1" d="M47.16 28.58A18.58 18.58 0 1 1 28.58 10a18.58 18.58 0 0 1 18.58 18.58zM54 54L41.94 42"></path></svg>
      <span class="mr-2 font-bold ">View Options</span> 
...
<Teleport to="body"  >
     <div v-if="open" class="relative z-950" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  <!--
    ...
    Leaving: "ease-in duration-200"
      ...
      To: "opacity-0"
  --> 
  <div @click="closeModal()" class="fixed inset-0 bg-black bg-opacity-75 transition-opacity"></div>

  <div class="fixed inset-0 z-10 overflow-y-auto mx-auto flex items-center max-w-full max-h-full justify-center" 
  :class="iframe_class" 
  > 
    <div class="flex relative items-center justify-center p-4 text-center sm:items-center sm:p-0">
      <div
        @click="closeModal()" 
        class="absolute bg-white circle flex right-0 text-black top-0 z-[1000] cursor-pointer" 
        
       >
      <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
      </div> 
      <!--
      ...
      -->    
      <div class="bg-white w-[400px] h-[80vh] h-full lg:w-[1200px] lg:h-[800px] block mx-auto" >
         
      <div class="mt-5 flex justify-center" role="status" v-if="loading" > 
      ...
      <span class="sr-only">Loading...</span>
        ...
        <iframe 
        @load="loading=false"  
        :src="iframe_src" class="w-full h-full "
           > 
  ...
  `,  
  props: ["variant-id", "data-option", "data-option-index","data-flavor-option-index","handle","iframe_src"] ,
  data() {
...
if (document.querySelector(".product-grid-section")) {
  ...
  const productForm = Vue.createApp({
    ...
    components: {  
      AddToCartSingle, 
      "product-quick-view":ProductQuickView
    },
    created() {},
  }).mount(productGrid);
