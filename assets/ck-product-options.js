  ...
  const productForm = Vue.createApp({
    ...
    data() {
      ...
      return {
        ...
        imageLoading: true,
        isAccordionOpen: [],
        isWaterBottleChecked:false,
        waterBottlePrice:"7.5"
      };
    },
    methods: {
      money(value) {
    ...
    computed: {
      ...
      subscriptionPrice() {
        ...
        let waterBottlePrice = this.waterBottlePrice
        if (this.isWaterBottleChecked && waterBottlePrice) {
          waterBottlePrice= waterBottlePrice*100;
          pPrice = price + waterBottlePrice;
        }  
        return this.money(pPrice);
      },
      currentPrice() {

        let pPrice= this.currentVariant.price;
        
        let waterBottlePrice = this.waterBottlePrice;
        if(this.isWaterBottleChecked){
          pPrice = pPrice + (waterBottlePrice*100); 
        }
        return this.money(this.currentVariant.price);
      },
      compareAtPrice() {
        return this.money(this.currentVariant.compare_at_price);
      }
      ,
      InstallementsPrice() {
        return this.money(this.currentVariant.price / 3);
      }
      ,
      selectedPrice() {
    ...
    watch: {
      currentVariant(newVariant, oldVariant) {
        ...
        if (
          ...
          this.option2.toLowerCase().includes("new")
        ) {
