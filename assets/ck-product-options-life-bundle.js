function handleize(inputString) {
  // This regex matches any character that is not a letter or a number
...
const DescriptionByOptionName = {

  prolonnutritionprogramsoupflavor_original:FlavorsDescriptionBYSKU["PR001"],
  prolonnutritionprogramsoupflavor_gen3:FlavorsDescriptionBYSKU["PR039"], 

  reset1dayfastingkitsoupflavor_variety1: FlavorsBYSKU["PR038"],
  reset1dayfastingkitsoupflavor_variety2: FlavorsBYSKU["PR055"],
  reset1dayfastingkitsoupflavor_variety3: FlavorsBYSKU["PR056"],
  reset1dayfastingkitsoupflavor_variety4: FlavorsBYSKU["PR057"],
...
<p></p></div></div>
   ...
   <button
      ...
      <a href="/cart" class="underline text-center w-full my-5 block">
      No Thanks 
      </a>
             </div> 
  ...
  const productForm = Vue.createApp({
    ...
    methods: {
      ...
      addToCart(id) {
        ...
        if (window.is_show_prolon_offer && showUpsellOffer=='true') {
      ...
      getProductBundleCheckedItems(t) {
        let custom_product_title = this.productSettings["custom_product_title"];
        if (custom_product_title) {
          let optionProductDescription = this.optionProductCheckedItems(
            "option_1",
            this.option1,
            1
          );
      ...
      optionProductCheckedItems(optionIndex, value, index) {
        let option = this.getOption(optionIndex, value, index);
        if (option) {
          return option["check_items"];
        } else {
          return null;
        }
      },
      ...
      optionFlavor(optionName,position){
        let OPtionNameHandle = handleize(optionName.toLowerCase());
        let selectedOption = handleize(this[`option${position}`].toLowerCase());
        let key = `${OPtionNameHandle}_${selectedOption}`;
        console.log(`key ${key}`); 
        let description = DescriptionByOptionName[key];
    ...
    computed: {
      ...
      productOptions() {
        ...
        // return options;
          ...
          return window.current_productOptions;
      },
      isShowBenefits() {
      ...
      saleMessage() {
        let totalOff= `15`;
        if(location.href.includes('holiday')){
          totalOff= `24`;
        } 
        // return '';
