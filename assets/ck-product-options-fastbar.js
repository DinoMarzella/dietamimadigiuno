...
if (document.querySelector("#ck-product-form")) {
  ...
  const productForm = Vue.createApp({
    ...
    data() {
      ...
      let option2 = window.selectedVariant.option2,
    ...
    methods: {
      ...
      directToCheckout() {
        let items = this.variantsToBuy.map((item) => {
          let variant = this.product.variants.find(
            (e) => e.option1 == item.option
          );
          ...
          if (this.plan == "recurring") {
            cartitem.selling_plan = this.selected_plan.id;
          }
      ...
      inStock(value){
        ...
        if(variant){
          return this.productAvailable[variant.id]; 
        }else{
          return false;
        }
      },
      ...
      addOptionToBuy(option) {
        if (option) {
          //VariantTobuy
          let variantToBuy = this.variantsToBuy.find((v) => {
            return v.option == option;
          });

          if (!variantToBuy) {
            this.variantsToBuy.push({
              option: option,
              quantity: 1,
            });
          }
      ...
      removeOptionToBuy(option) {
        if (option) {
          ...
          if (variantToBuy) {
            this.variantsToBuy.splice(
              this.variantsToBuy.indexOf(variantToBuy),
              1
            );
          }
        }
      },
      increaseOptionToBuy(option) {
        if (option) {
          //VariantTobuy
          let variantToBuy = this.variantsToBuy.find((v) => {
            return v.option == option;
          ...
          if (variantToBuy) {
            variantToBuy.quantity++;
          }
        }
      },
      decreaseOptionToBuy(option) {
        if (option) {
          //VariantTobuy
          let variantToBuy = this.variantsToBuy.find((v) => {
            return v.option == option;
          ...
          if (variantToBuy) {
            if (variantToBuy.quantity > 1) {
              variantToBuy.quantity--;
            } else {
              this.removeOptionToBuy(option);
            }
          }
        }
      },
      isOptionToBuyAdded(option) {
        if (option) {
          //VariantTobuy
          let variantToBuy = this.variantsToBuy.find((v) => {
            return v.option == option;
      ...
      getOptionToBuyQuantity(option) {
        if (option) {
          ...
          if (variantToBuy) {
            return variantToBuy.quantity;
          }
        }
        return 0;
      },

      getVariantOption(option) {
        let variants = this.product.variants;
        let variant = variants.find((v) => {
          return v.option1 == option;
        });
        return variant;
      },
    ...
    computed: {
      ...
      subscriptionPricePerBar() {
        ...
        let perBarPrice = price / this.bar_count;
        let money = this.money(perBarPrice);
        let perBarPriceText = money + " / Bar";
        return perBarPriceText;
      },
      currentPrice() {
        return this.money(this.currentVariant.price);
      },
      currentPricePerBar() {
        let perBarPrice = this.currentVariant.price / this.bar_count;
        let money = this.money(perBarPrice);
        let perBarPriceText = money + " / Bar";
        return perBarPriceText;
      },
      getTotalBuyVariantPrice() {
        let total = 0;
        this.variantsToBuy.forEach((variantToBuy) => {
          let variant = this.getVariantOption(variantToBuy.option);
          if (variant) {
            let price = variant.price;
            if (this.plan == "recurring") {
              let plan = variant.selling_plan_allocations;

              if (plan && plan[0] && plan[0].price) {
                price = plan[0].price;
              }
            }

            total += price * variantToBuy.quantity;
          }
        });

        return total;
      },
      getTotalBuyVariantOneTimePrice() {
        if (this.plan == "recurring") {
          let total = 0;
          this.variantsToBuy.forEach((variantToBuy) => {
            let variant = this.getVariantOption(variantToBuy.option);
      ...
      savingText() {
        if (this.getTotalBuyVariantOneTimePrice) {
          let saving =
            this.getTotalBuyVariantOneTimePrice - this.getTotalBuyVariantPrice;
          if (saving > 0) {
            let money = this.money(saving);
      ...
      selling_plan() {
        if (this.plan == "recurring") return this.selected_plan.id;
        else return null;
      },
      allowedToPurchaseVariantBox() {
        
        let variantsToBuy = this.variantsToBuy;
        if (variantsToBuy.length > 0) {
          return true;
        }
        return false;
      }, 
      ADD_TO_CART_TEXT: function () {
        if (
          this.product &&
          this.product.tags &&
          this.product.tags.includes("preorder")
        ) {
          return "Pre Order";
        } else {
          return "Add to Cart";
    ...
    watch: {
      currentVariant(newVariant, oldVariant) {
        ...
        if (newVariant && newVariant.option1 == "ProLon 3-Day Reset") {
          this.option2 = "Original";
        }
        ...
        if (newVariant.image_id) {
          image = this.product.images.find((image) => {
            return image.id == newVariant.image_id;
          });
        }
        if (newVariant.featured_image && newVariant.featured_image.src) {
